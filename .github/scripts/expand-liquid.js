...
async function expandExtensionLiquidTemplates(projectName, flavor) {
  ...
  const pathSuffix = flavor === "typescript" || flavor === "vanilla-js" ? "js" : "rs";
  const projectPath = path.join(process.cwd(), projectName + "-" + pathSuffix);
  const langName = flavor === "typescript" || flavor === "vanilla-js" ? "javascript" : "rust";

  if (langName === "javascript") {
    await (await glob(path.join(projectPath, 'src', '!(*.liquid|*.graphql)'))).forEach(async (path) => await fs.rm(path));
  }
  ...
  if (langName === "javascript") {
    const srcFilePaths = await glob(path.join(projectPath, 'src', '!(*.liquid|*.graphql)'))
